  ...
  return (
    <div
      className={`px-4 sm:px-12 py-4 sticky top-0 z-50 ${
        isOpen
          ? 'bg-[#000a0a]' // Fondo fijo cuando el menú está abierto
          : isScrolled
          ? 'bg-[#000a0a]/50 backdrop-blur-lg transition duration-300'
          : 'bg-transparent'
      }`}
    >
      ...
      {isMobileOrTablet && isOpen && (
        <div
          className={`fixed top-0 left-0 w-full h-full bg-[#000a0a] text-white flex flex-col justify-between z-10 ${isMobile ? 'px-4' : isTablet ? 'px-12' : ''} ${isMobile || isTablet ? 'pt-[120px]' : ''}`}
        >
          {/* Parte superior */}
